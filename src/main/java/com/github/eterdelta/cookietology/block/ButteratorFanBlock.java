package com.github.eterdelta.cookietology.block;
    ...
    public void animateTick(BlockState blockState, Level level, BlockPos blockPos, Random random) {
        if (level.getBlockState(blockPos).is(CookietologyBlocks.BROKEN_BUTTERATOR_FAN.get())) {
            if (random.nextDouble() <= 0.4D) {
                level.addParticle(ParticleTypes.LARGE_SMOKE, blockPos.getX() + random.nextDouble(0.1D, 0.7D), blockPos.getY() + 1.0D, blockPos.getZ() + random.nextDouble(0.1D, 0.7D), 0.0D, 0.0D, 0.0D);
            }
        }
    }
}